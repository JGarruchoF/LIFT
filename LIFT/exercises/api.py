...
def create_exercise(request, item: CreateExerciseSchema):
    target = (
        get_object_or_404(Muscle, pk=item.target) if item.target is not None else None
    )
    equipment, _ = (
        Equipment.objects.get_or_create(name=item.equipment_name)
        if item.equipment_name is not None
        else None
    )
    try:
        created_item = Exercise.objects.create(
            name=item.name, target=target, equipment=equipment, gif_url=item.gif_url
        )
        return created_item
